...
import { AllCommentAndGetReplyUserQuery } from './getAllCommentAndReply.query';
import { ConditionEnum } from 'src/Domain/entities/enums/condition.enum';

@QueryHandler(AllCommentAndGetReplyUserQuery)
export class AllCommentAndGetReplyUserQueryHandler implements IQueryHandler<AllCommentAndGetReplyUserQuery> {
  constructor(
  ...
  async execute(query: AllCommentAndGetReplyUserQuery): Promise<any> {
    try {
