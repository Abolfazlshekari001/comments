...
const errResult = smsError(
  axiosResponse.status,
  axiosResponse.data.message,
);
jest.spyOn(sendSmsService, 'sendSms').mockImplementation(async () => {
  throw new HttpException(errResult.message, errResult.status_code);
});
await expect(sendSmsService.sendSms(mockDto)).rejects.toThrow(
  new HttpException(errResult.message, errResult.status_code),
);
expect(sendSmsService.sendSms).toHaveBeenCalledWith(mockDto);